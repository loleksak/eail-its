...
elm_main(int argc, char **argv) {
	...
	elm_win_title_set(win, "Video app");
	...
	register_request_listener(EVENT_HIDE, hide, video);
	register_request_listener(EVENT_SHOW, show, video);

	evas_object_resize(win, 240, 60);
	evas_object_resize(video, 120, 30);
	evas_object_move(video, 60, 15);
